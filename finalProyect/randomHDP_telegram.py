...
def createRoom(update,context):
    ...
    players={}
    rooms[context.args[0]]=[context.args[0],cartasBlancas,cartasNegras, update.effective_chat.id, 0, players]
    rooms[context.args[0]][-1][context.args[1]] = update.effective_chat.id
...
def joinRoom(update,context):
    ...
    if context.args[0] in rooms:
        rooms[context.args[0]][-1][context.args[1]] = update.effective_chat.id
        pprint(rooms[context.args[0]])
        context.bot.send_message(chat_id=update.effective_chat.id, text=f"Succesfully joint to room " + str(context.args[0]))
        context.bot.send_message(chat_id=rooms[context.args[0]][-3], text=context.args[1] + " joined the room!")
        return 0
